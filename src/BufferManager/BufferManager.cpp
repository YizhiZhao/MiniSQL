...
blockNode* BufferManager::get_Block(fileNode *file, blockNode *position, bool if_pin)
{
    ...
    string fullFileName = DATA_FOLDER + filename;
    //read the file content to the block
    FILE * fp = fopen(fullFileName.c_str(), "ab+");//appendix read and write binary file
    ...
    else // written back to the file
    {
        ...
        if(fp != nullptr)//open successfully
        {
            if(fseek(fp, block->offsetNum * BLOCK_SIZE, 0) == 0)//seek successfully
            {
                ...
                {
                    cout << "There are some problems writting the file "<< fullFileName << " in write back to disk!" << endl;
                    exit(0);
                }
            }
            else//seek failed
            {
                cout << "There are some problems seeking the file "<< fullFileName << " in write back to disk!" << endl;
                exit(0);
            }
            fclose(fp);
        }
        ...
        {
            cout << "There are some problems opening the file "<< fullFileName << " in write back to disk!" << endl;
            exit(0);
        }
    }
}
