...
protected:
	...
	std::vector<T> keys;		   // ascending order && distinct
};
...
private:
	std::vector<BPTNodeBase<T, DEGREE> *> children; // internal or leaf
};
...
#endif
private:
	std::vector<int> keysOffset;   		 // keysOffset in .db file
...
class BPTree {
public:
	BPTree(const std::string& fileName, int typeSize);
	~BPTree();
...
private:
	...
	std::string fileName;      // fileName == indexName
...
template <typename T, size_t DEGREE>
BPTree<T, DEGREE>::BPTree(const std::string& fileName, int typeSize) :
...
void BPTree<T, DEGREE>::readBlock(blockNode *bi)
{
	...
	while(offsetBegin + sizeof(int) <= reinterpret_cast<unsigned char *>(bm.get_content(*bi)) + bm.get_UsingSize(*bi)) {
		...
		std::string tag = "class std::basic_std::string<char,struct std::char_traits<char>,class std::allocator<char> >";
